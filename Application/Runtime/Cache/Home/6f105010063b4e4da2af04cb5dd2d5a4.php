...
<div class="buy-wrap">
    ...
    <!--buy-process end-->
    <form id="cartFrom" onsubmit="return false;">
        <!--order-confirm begin-->
        <div class="order-confirm">
            ...
            <div class="buy-info">
                <div class="buy-title">
                    <h2>基本信息</h2>
                    <span>我的交易员：<?php echo ($admin["name"]); ?>  <?php echo ($admin["mobile"]); ?></span>
                </div>
                <table id="table">
                    <tr>
                        <td width="80">客户名称：</td>
                        <td width="200"><?php echo ($_SESSION['uinfo']['c_name']); ?></td>
                        <td width="80">联<i></i>系<i></i>人：</td>
                        <td width="110"><?php echo ($_SESSION['uinfo']['con_name']); ?></td>
                        <td width="80"></td>
                    </tr>
                    <tr>
                        <td>联系电话：</td>
                        <td><?php echo ($_SESSION['uinfo']['mobile']); ?></td>
                        <td></td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <select datatype="*" name="pay_method" class="required" id="pay_method">
                                ...
                                <?php if(is_array($pay_method)): foreach($pay_method as $key=>$value): ?><option value="<?php echo ($key); ?>"><?php echo ($value); ?></option><?php endforeach; endif; ?>
                            </select>
                        </td>
                        ...
                        <td>
                            <select datatype="*" id="transport_type" name="transport_type" class="required">
                                <option value="">--请选择--</option>
                                <?php if(is_array($transport_type)): foreach($transport_type as $key=>$value): ?><option value="<?php echo ($key); ?>"><?php echo ($value); ?></option><?php endforeach; endif; ?>
                            </select>
                        </td>
                    ...
                    <tr class="other hide">
                        ...
                        <td>
                            <select datatype="*" name="delivery_place" class="required" id="delivery_place" >
                                <option value="">请选择</option>
                                <?php if(is_array($area)): foreach($area as $key=>$value): ?><option value="<?php echo ($value["id"]); ?>"><?php echo ($value["name"]); ?></option><?php endforeach; endif; ?>
                            </select>
                        </td>
                        <td>详细地址<span class="red">*</span>:</td>
                        <td><input type="text" id="address"  datatype="*" name="address" class="required"  placeholder="详细地址" maxlength="50"/></td>
                    </tr>
                </table>
                <!--sly-chn begin-->
                <div class="sly-chn">
        ...
        <!--order-confirm end-->
        <!--detail begin-->
        <div class="detail" id="load-cart" data-url="Home/Cart/loadCart">
			<!-- loada的模块 -->
        </div>
        <div class="remark">
            <!--remark-lt begin-->
...
$(function(){
    ...
    $('#transport_type').on('click',function(){
        if($(this).val()>1){
            $('#table .other').removeClass('hide');
        }else{
            $('#table .other').addClass('hide');
        }
    });
    //提交表单
    $("#cartFrom").on('submit',function(){
        ...
        $.post('/Home/Cart/addorder',data,function(res){
            if(res.err==0){
                layer.msg("提交成功",1,1);
...
var check = function(){
    ...
    //
    if(pay_method==''){
        layer.msg('支付方式不能为空',1,3);
        pay_method.focus();
        rtn = false;
        return rtn;
    }
    if(transport_type==''){
        layer.msg('运输方式不能为空',1,3);
        transport_type.focus();
        rtn = false;
        return rtn;
    }
    if(date3==''){
        layer.msg('配送时间不能为空',1,3);
        date3.focus();
        rtn = false;
        return rtn;
    }
