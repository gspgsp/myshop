...
{	
		...
		if($user)
		{
			...
			if($user['id'] == 1 || $user['is_use'] == 1)
			{
				...
				if($user['password'] == md5($password.C('MD5_KEY')))
				{
					...
					if(session('id') == 0 || session('username') == '' || session('username') == 'undefined'){
						startHomeSession();
						$redis = new RedisCluster();
						$orgredis = $redis->getRedis();
						$orgredis->set('id'.SESS_ID,$user['id']);
						$orgredis->set('username'.SESS_ID,$user['username']);
						session('id',$orgredis->get('id'.SESS_ID));
						session('username',$orgredis->get('username'.SESS_ID));
					}
					return TRUE;
				}
				else 
	...
	public function search($pageSize = 20)
	{
		...
		$is_use = I('get.is_use');
		if($is_use != '' && $is_use != '-1')
			$where["a.is_use"] = array('eq', $is_use);
		/************************************* 翻页 ****************************************/
