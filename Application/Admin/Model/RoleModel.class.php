...
class RoleModel extends Model 
{
	...
	public function search($pageSize = 20)
	{
		...
		if($role_name = I('get.role_name'))
			$where["r.role_name"] = array('like', "%$role_name%");
		/************************************* 翻页 ****************************************/
...
=c.id GROUP BY a.id;*/
		...
		// ->alias('a')
		// ->join('LEFT JOIN php34_role_privilege b ON a.id=b.role_id LEFT JOIN php34_privilege c ON b.pri_id=c.id')
		// ->where($where)
		// ->group('a.id')
		// ->limit($page->firstRow.','.$page->listRows)
		// ->select();
		$data['data'] = $this->field('r.*,GROUP_CONCAT(p.pri_name) pri_name')
		->alias('r')
		->join('__ROLE_PRIVILEGE__ rp ON r.id = rp.role_id','left')
		->join('__PRIVILEGE__ p ON p.id = rp.pri_id','left')
		->where($where)
		->group('r.id')
		->limit($page->firstRow.','.$page->listRows)
		->select();
